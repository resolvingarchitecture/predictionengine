...
{
	public static final Logger LOG = Logger.getLogger(DatabasePortal.class.getName());
...
//				groups[groupIndex.getIndex(g)].value_fargID = i;
	...
	public boolean loadGroupByUserMembership(Connection con)
	{
		...
		try
		{
			...
			LOG.warning("LoadGroupUserMembership SQLException: " + ex.getMessage());
		}
		return (c>0);
	}
	...
	public void loadUserRatings(Connection con, int start, int length) {
		...
		try
		{
	        ...
	        System.out.println("Querying User Ratings ("+mine.getUserID(start)+" - "+mine.getUserID(start+length-1)+")--------");
			ResultSet rs = db.Query(con,"exec usp_DM_user_Rating_Select_Range "+mine.getUserID(start)+", "+mine.getUserID(start+length-1));
